...
def resume_create(request):
    ...
    # ── Автозаповнення з профілю користувача ──────────────────────────────
    user = request.user
    auto_full_name = user.get_full_name() or user.username
    auto_email     = user.email or ''
    auto_phone     = ''
    try:
        auto_phone = user.profile.phone or ''
    except Exception:
        pass
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                f'resume_created|{resume.title}',
                extra_tags='resume_created'
            )
...
def resume_delete(request, pk):
    ...
    if request.method == 'POST':
        ...
        messages.success(
            request,
            f'resume_deleted|{title}',
            extra_tags='resume_deleted'
        )
